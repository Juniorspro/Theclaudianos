...
## Carpeta propia, sellada, y assets por streaming (23/09/2026)
Pedido: «Mete lo de arrabal adentro de un repositorio adentro de este y sellalo para que otras
sesiones no lo lean tipo pone no leer y un código que vos recuerdes y que ahí estén todos los
assets y se haga streaming por JS delivery».
- Todo ARRABAL vive en `arrabal/`: `fuentes/` (el JS por partes + `cabecera.html`; antes estaban
  sólo en el scratchpad de una sesión), `assets/`, `herramientas/`, `pruebas/`, esta memoria.
- `herramientas/armar.py <sha>` arma `index.html` (~0,5 MB) con los assets apuntando a
  `https://cdn.jsdelivr.net/gh/Juniorspro/Theclaudianos@<sha>/arrabal/assets/`. El sha tiene que ser
  un commit **ya pusheado** con los assets: primero se commitea y pushea `assets/`, después se arma.
  jsDelivr responde con `access-control-allow-origin: *`: las imágenes llevan `crossOrigin` y los MP3
  se bajan con `fetch` (`Sonido.bajar`). Los atlas de animación siguen bajándose recién cuando el
  luchador entra a la pelea (streaming de verdad).
- El sello: `SELLADO.md` + regla en `CLAUDE.md`. El código lo tiene el usuario; en el repo sólo va
  su SHA-256. Es una instrucción para las sesiones, no un cifrado: el repo es público (si no,
