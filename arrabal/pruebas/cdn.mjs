...
// salida, así que los pedidos a jsDelivr se bajan con curl (que verifica con el CA del entorno)
// y se le entregan a la página tal cual, con el CORS que manda jsDelivr. En un teléfono no hace falta.
...
export async function usarCDN(ctx){
  await ctx.route('https://cdn.jsdelivr.net/**',async route=>{
    ...
    try{const body=await bajar(u);
      const tipo=u.endsWith('.avif')?'image/avif':u.endsWith('.webp')?'image/webp':u.endsWith('.png')?'image/png':u.endsWith('.mp3')?'audio/mpeg':'application/octet-stream';
