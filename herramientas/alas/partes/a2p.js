...
const POST = {ok:false, rt:null, msaa:0, mips:[], w:0, h:0, bloom:true, t:0, danio:0, cegado:0, lento:0, flash:0, apagon:0, solUV:new THREE.Vector2(-9,-9), solVis:0, solCol:new THREE.Vector3(1,0.95,0.85),
...
const MAT_FINAL = new THREE.ShaderMaterial({
  uniforms:{tHdr:{value:null}, tBloom:{value:null}, bloomK:{value:0.45}, expo:{value:1}, tiempo:{value:0}, res:{value:new THREE.Vector2(1,1)},
    ...
    danio:{value:0}, cegado:{value:0}, lento:{value:0}, usaBloom:{value:1}, apagon:{value:0}, solUV:{value:new THREE.Vector2(-9,-9)}, solVis:{value:0}, solCol:{value:new THREE.Vector3(1,1,1)}, aspecto:{value:2}},
  vertexShader:VERT_Q, depthTest:false, depthWrite:false,
  fragmentShader:`uniform sampler2D tHdr, tBloom; uniform float bloomK, expo, tiempo, sat, contraste, vineta, grano, aberr, danio, cegado, lento, usaBloom, apagon, solVis, aspecto; uniform vec2 res, solUV; uniform vec3 solCol;
    ...
    void main(){
      ...
      /* destello de lente del sol: halo, raya anamórfica y fantasmas sobre la línea sol-centro (sólo si el sol se ve) */
      if(solVis > 0.001){ vec2 q = (vUv - solUV)*vec2(aspecto, 1.0); float dq = length(q);
        vec3 fl = solCol*(0.45*exp(-dq*16.0) + 0.05*exp(-dq*4.0)) + solCol*vec3(0.55,0.75,1.0)*0.5*exp(-abs(q.y)*90.0)*exp(-abs(q.x)*2.2);
        vec2 eje = vec2(0.5) - solUV;
        for(int k=1;k<5;k++){ float f = float(k)*0.45 - 0.2; vec2 p = (vUv - (solUV + eje*(1.0 + f)))*vec2(aspecto, 1.0); float rr = 0.02 + 0.025*float(k);
          fl += mix(vec3(0.35,0.6,1.0), vec3(1.0,0.55,0.25), float(k)/4.0)*0.07*smoothstep(rr, rr*0.7, length(p)); }
        c += fl*solVis; }
      ...
      /* fuerza G sostenida: se cierra la vista desde los bordes y se destiñe */
      float ap = clamp(apagon, 0.0, 1.0); c = mix(c, vec3(dot(c, vec3(0.3,0.5,0.2))), ap*0.6); c *= 1.0 - ap*smoothstep(0.02 + (1.0 - ap)*0.4, 0.5, r2*2.2);
      c = clamp(c, 0.0, 1.0);
      c = aSRGB(c);
...
function revelar(dibujarEscena){
  ...
  U.danio.value = POST.danio; U.cegado.value = POST.cegado; U.lento.value = POST.lento; U.apagon.value = POST.apagon; U.solUV.value.copy(POST.solUV); U.solVis.value = POST.solVis; U.solCol.value.copy(POST.solCol); U.aspecto.value = POST.w/POST.h;
...
function matBarato(m){
  ...
  /* un material con shader propio (la isla mezcla cinco texturas) se lleva el mismo parche, que ya trae la niebla */
  if(m.userData && m.userData.propio){ b.onBeforeCompile = m.onBeforeCompile; b.customProgramCacheKey = m.customProgramCacheKey; b.userData.niebla = true; }
  else if(m.userData && m.userData.niebla) parcheNiebla(b);
