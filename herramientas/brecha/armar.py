#!/usr/bin/env python3
"""Arma BRECHA 7: junta las partes y mete los assets procesados (manifiestos de gen/listo) adentro del HTML.
...
GEN = os.environ.get('BRECHA_GEN', '/tmp/claude-0/b7/gen/listo')
SALIDA = os.path.normpath(os.path.join(AQUI, '..', '..', 'juegos-pc', 'Brecha.html'))
PARTES = ['p1.html', 'p1b.html', '@assets', 'p2.js', 'p2s.js', 'p2i.js', 'p2c.js', 'p3.js', 'p3p.js', 'p3m.js', 'p3s.js', 'p3b.js', 'p4.js', 'p4b.js', 'p5a.js', 'p5.js']
MIME = {'.webp': 'image/webp', '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.mp3': 'audio/mpeg', '.glb': 'model/gltf-binary', '.wav': 'audio/wav'}


CACHE = os.environ.get('BRECHA_CACHE', '/tmp/claude-0/b7/gen/cache')
...
def armar(solo=None, sin=False, salida=SALIDA):
    ARCH, MAN, peso = {}, {k: [] for k in ('tex', 'cielo', 'arte', 'mod', 'pj', 'aud')}, {}
    quiere = lambda k: not sin and (not solo or k in solo)

    ARMAS_JUG = {'arma_p9', 'arma_k5', 'arma_r4', 'arma_b12', 'arma_m14', 'arma_l96'}
    ...
    if quiere('cielo'):
        ...
        GIRO_AZ = {'cielo_puerto': 233, 'cielo_autopista': 329}
        for e in leer('cielo'):
            ruta = e.get('archivo')
            if ruta and not os.path.isabs(ruta):
                ruta = os.path.join(GEN, ruta)
    ...
    for cat in ('mod', 'pj'):
        ...
        for e in leer(cat):
            ...
            if cat == 'mod' and e['id'].startswith('arma_') and mete('glb/%s_lod' % e['id'], e.get('glb'), cat):
