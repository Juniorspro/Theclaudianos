...
(async () => {
  ...
  await p.goto('file:///tmp/ui/linea_b.html'); await p.waitForTimeout(800); await p.evaluate(d => window.__dif = d, process.argv[4] || 'normal');
  const inv = process.argv[2] !== 'mortal', caps = (process.argv[3] || '0,1,2,3,4').split(',').map(Number);
  for(const c of caps){
    const r = await p.evaluate(([c, inv]) => {
      const L = __L; L.AJ.visto = true; L.AJ.dificultad = window.__dif || 'normal'; L.empezarCap(c);
      ...
      window.__BOT = (C) => { const J = L.J, P = L.JUG; C.mx = C.my = 0; C.apunta = false; C.tira = false;
        ...
        if(!P.arma || (L.ARMAS[P.arma].balas && P.balas === 0) || (gordo && !sirve(P.arma))){ const it = L.ITEMS.filter(i => !i.vuela && (!L.ARMAS[i.k].balas || i.balas > 0) && (!gordo || sirve(i.k))).sort((u, v) => Math.hypot(u.x - P.x, u.y - P.y) - Math.hypot(v.x - P.x, v.y - P.y))[0];
          if(a && a.k === 'agarrar' && (L.ARMAS[a.it.k].melee || a.it.balas > 0) && (!P.arma || P.balas === 0 || (gordo && !sirve(P.arma) && sirve(a.it.k)))){ L.hacerAccion(); return; }
        ...
        if(vivos.length){ const fJ = L.campo(P.x, P.y), w = L.PISO.w; let e = null, md = 1e9;
          ...
          if(A && !A.melee && P.balas === 0 && ve && d < 110){ C.apunta = true; C.ax = e.x - P.x; C.ay = e.y - P.y; P.ang = Math.atan2(C.ay, C.ax); L.tirarArma(); return; }
