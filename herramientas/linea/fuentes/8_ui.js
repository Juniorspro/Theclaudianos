...
/* GOLPEAR, a la izquierda de la palanca derecha: pega (o dispara) hacia donde mirás, y sostenido sigue pegando */
function centroGolpe(){ const c = centroPalo('palo'); return {x:c.x - R_PALO - 27, y:c.y - 5, r:15}; }
...
stage.addEventListener('pointerdown', ev => {
  ...
  const gp = centroGolpe(); if(Math.hypot(x - gp.x, y - gp.y) < gp.r + 6){ DEDOS.set(ev.pointerId, {tipo:'golpe'}); return; }
...
function pasoPalancas(dtR){
  ...
  /* el botón GOLPEAR (o la F): si no estás apuntando, busca solo al más cercano que se ve al alcance */
  CTRL.golpe = [...DEDOS.values()].some(d => d.tipo === 'golpe') || !!T.KeyF;
  if(CTRL.golpe){ CTRL.tira = true;
    if(!CTRL.apunta){ const A = JUG.arma && ARMAS[JUG.arma], alc = !A ? 40 : A.melee ? A.alcance + 30 : 230; let mejor = null, md = alc;
      for(const e of ENEM){ if(e.muerto) continue; const d = Math.hypot(e.x - JUG.x, e.y - JUG.y); if(d < md && seVe(JUG.x, JUG.y, e.x, e.y)){ md = d; mejor = e; } }
      CTRL.apunta = true; if(mejor){ CTRL.ax = mejor.x - JUG.x; CTRL.ay = mejor.y - JUG.y; } else { CTRL.ax = Math.cos(JUG.ang); CTRL.ay = Math.sin(JUG.ang); } } }
...
function palancas(g){
  ...
  /* GOLPEAR */
  const gp = centroGolpe(), ap = CTRL.golpe, Ag = JUG.arma && ARMAS[JUG.arma], dy = ap ? 1 : 0;
  g.globalAlpha = ap ? 1 : 0.8;
  disco(g, gp.x, gp.y + 1, gp.r + 1, K); disco(g, gp.x, gp.y + dy, gp.r, ap ? '#ff4a2a' : '#c8281a'); disco(g, gp.x - 2, gp.y - 3 + dy, gp.r - 6, ap ? '#ff9a6a' : '#ff5a3a');
  if(Ag && !Ag.melee){ const cx = gp.x, cy = gp.y + dy; anillo(g, cx, cy, 5, '#ffffff'); rect(g, cx - 1, cy - 8, 2, 4, '#ffffff'); rect(g, cx - 1, cy + 5, 2, 4, '#ffffff'); rect(g, cx - 8, cy - 1, 4, 2, '#ffffff'); rect(g, cx + 5, cy - 1, 4, 2, '#ffffff'); }
  else { const cx = gp.x - 4, cy = gp.y - 4 + dy; rect(g, cx, cy + 1, 8, 6, K); rect(g, cx + 1, cy, 7, 6, '#f4d8b8'); for(let i = 0; i < 3; i++) rect(g, cx + 3 + i*2, cy, 1, 3, '#c89878'); rect(g, cx - 1, cy + 2, 3, 3, '#f4d8b8'); }
  g.globalAlpha = 1;
  texto(g, Ag && !Ag.melee ? tr('DISPARAR') : tr('GOLPEAR'), gp.x, gp.y + gp.r + 3, 'blanco');
...
const PISTAS = [
  ...
  {txt:'APUNTÁ CON LA DERECHA Y PEGÁ CON GOLPEAR', hecho:() => J.pisteo.pego},
