...
function blancoDir(ax, ay){
  const ox = HE.x, oy = HE.y + 1.25, a0 = Math.atan2(ay, ax); let mejor = null, md = 0.45;
  const dif = a => { let d = Math.abs(a - a0) % (Math.PI*2); return d > Math.PI ? Math.PI*2 - d : d; };
  const probar = (x, y, cosa, yc) => { const d = Math.hypot(x - ox, y - oy); if(d > 24 || d < 0.3) return; const s = dif(Math.atan2(y - oy, x - ox)) + d*0.004;
    if(s < md && lineaBala(ox, oy, x, y)){ md = s; mejor = {cosa, cabeza:yc !== undefined && dif(Math.atan2(yc - oy, x - ox)) < Math.min(0.06, dif(Math.atan2(y - oy, x - ox)))}; } };
  for(const e of ENEM) if(!e.muerto && enPantalla(e.x, e.y)) probar(e.x, e.y + e.alto*0.6, e, e.y + e.alto*0.9);
  for(const b of BALAS) if(b.tetera && b.vida > 0) probar(b.x, b.y, b);
  for(const o of OBJ) if(o.vivo && o.tipo !== 'puerta' && enPantalla(o.x, o.y)) probar(o.x, o.y - 0.4, o);
  return mejor;
}
/* tirar hacia donde apunta la palanca, sin blanco */
function dispararDir(ax, ay){
  const p = {x:HE.x + ax*10, y:HE.y + 1.25 + ay*10 + 0.4, vivo:true}; HE.obj = [p, p];
  disparar({cosa:{x:p.x, y:p.y - 0.4 - 0.1, vivo:true, libre:true}, cabeza:false});
}
function pasoApunte(dtR){
  const C = CTRL; if(!PAL() || HE.muerto || J.fin){ C.blanco = null; return; }
  if(!C.apunta){ C.blanco = null; C.t = Math.min(C.t, 0.05); return; }
  const n = Math.hypot(C.ax, C.ay) || 1, ax = C.ax/n, ay = -C.ay/n;
  C.blanco = blancoDir(ax, ay);
  if(C.blanco){ HE.obj = [C.blanco.cosa, C.blanco.cosa]; } else { const p = {x:HE.x + ax*10, y:HE.y + 1.25 + ay*10 + 0.4, vivo:true}; HE.obj = [p, p]; }
  if(Math.abs(ax) > 0.15) HE.dir = sig(ax);
  C.t -= dtR*(0.55 + 0.45*J.ts);
  if(C.t <= 0){ C.t = 0.16; if(C.blanco) disparar(C.blanco); else dispararDir(ax, ay); }
}
...
function disparar(b){
  ...
  if(!c.libre){ HE.obj[i] = c; if(HE.obj[i ^ 1] === null || (HE.obj[i ^ 1].muerto || HE.obj[i ^ 1].vivo === false)) HE.obj[i ^ 1] = c; }
