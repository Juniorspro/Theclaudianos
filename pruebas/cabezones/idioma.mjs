...
await pg.evaluate("for(let i=0;i<45;i++){__H.anda(1);__H.dibujarYa();}");
const abierto=await pg.evaluate("Idioma.abierto&&Idioma.primera");
await pg.screenshot({path:(vert?'v':'h')+'-idioma.png'});
// tocar "ENGLISH" con un toque real: el rectángulo en unidades de diseño -> ventana
const vw=await pg.evaluate("(()=>{var r=Idioma.rects.find(x=>x.id==='en');return __H.aVentana(r.x+r.w/2,r.y+r.h/2);})()");
await toque(vw);await pg.evaluate("__H.anda(5)");
const idi=await pg.evaluate("Idioma.actual");const guardado=await pg.evaluate("localStorage.getItem('cabezones.idioma')");
const pantAntes=await pg.evaluate("__H.pant()");
await pg.evaluate("__H.anda(60);__H.dibujarYa()");await pg.screenshot({path:(vert?'v':'h')+'-portada-en.png'});
// el globo de la portada vuelve a abrir la pantalla
const gb=await pg.evaluate("__H.aVentana(Idioma.boton.x,Idioma.boton.y)");
await toque(gb);await pg.evaluate("__H.anda(20);__H.dibujarYa()");const reabre=await pg.evaluate("Idioma.abierto&&__H.pant()==='portada'");
const rp=await pg.evaluate("(()=>{var r=Idioma.rects.find(x=>x.id==='pt');return __H.aVentana(r.x+r.w/2,r.y+r.h/2);})()");
await toque(rp);await pg.evaluate("__H.anda(5)");
for(const l of ['pt','fr','en']){await pg.evaluate(l=>{Idioma.elegir(l);},l);
  for(const p of ['menu','vestuario','torneos']){await pg.evaluate(p=>{__H.ir(p);__H.anda(60);__H.dibujarYa();},p);await pg.screenshot({path:(vert?'v':'h')+'-'+l+'-'+p+'.png'});}}
